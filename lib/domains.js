...
const DOMAIN_RULES = Object.freeze({
  morph: {
    ...
    exportable: false,
    deletable: true,
    maxDepth: 1,  // NO INCEPTION CASCADE — 1 layer deep, no dream within a dream
    description: 'The dreamscape. Mutable, experimental, sacred. Never shared externally. Max depth: 1.',
    tetragrammaton: 'Yod',
    element: 'Fire',
    letter: 'י'
  },
...
// Morph depth tracking — NO INCEPTION CASCADE
let morphDepth = 0;
const MORPH_MAX_DEPTH = 1;
...
function write(domain, name, content, metadata = {}) {
  ...
  // DREAMSPACE DEPTH LIMIT — 1 layer only, no dream within a dream
  if (domain === 'morph') {
    if (morphDepth >= MORPH_MAX_DEPTH) {
      throw new Error(`INCEPTION VIOLATION: morph depth ${morphDepth + 1} exceeds limit of ${MORPH_MAX_DEPTH}. No dream within a dream.`);
    }
    morphDepth++;
  }
