...
const SCHEMA_DIR = path.join(__dirname, '..', 'schema', 'v1');

// Load schemas — schema/v1 is executable truth
const commonLingua = JSON.parse(fs.readFileSync(path.join(SCHEMA_DIR, 'common-lingua.schema.json'), 'utf8'));
...
function parseFile(filePath) {
  ...
  // For .flow and .tool, use standard YAML — surface parse errors with context
  // instead of letting a raw YAMLException bubble up as a stack trace.
  try {
...
function validate(obj, type) {
  // Only 'flow' and 'tool' are validatable. Anything else (e.g. detectType
  // returned null for an unknown extension) must be rejected explicitly rather
  // than silently falling through to the tool validator.
  if (type !== 'flow' && type !== 'tool') {
    return {
      valid: false,
      errors: [{
        path: '/',
        message: `Cannot validate: unknown L7 type${type ? ` "${type}"` : ''} (expected "flow" or "tool")`,
        params: {}
      }]
    };
  }

  if (type === 'tool' && process.env.L7_REQUIRE_DECLARED === '1' && !(obj && obj.l7)) {
    return {
      valid: false,
      errors: [{
        path: '/l7',
...
/**
 * List all files of a given type (host/ first, then L7_DIR prefix)
 */
function listFiles(type) {
  const catalog = require('./catalog');
  const listed = type === 'flow' ? catalog.listFlows() : catalog.listTools();
  if (listed.length > 0) return listed;

  // Legacy .l7 files in the prefix root
  if (fs.existsSync(L7_DIR)) {
    return fs.readdirSync(L7_DIR)
      .filter(f => f.endsWith('.l7'))
      .map(f => ({
        name: path.basename(f, '.l7'),
        path: path.join(L7_DIR, f),
        legacy: true
      }));
  }
  return [];
...
/**
 * Coerce a replacement for mixed-string templates.
 * Never produce "[object Object]" — JSON-encode records/arrays instead.
 */
function replacementToString(value) {
 ...
 *
 * Non-string values (number, bool, object, array) pass through unchanged.
 * A template that is exactly `$name`, `$a.field`, or `{{ expr }}` returns the
 * resolved value as-is — objects/arrays are not String()'d.
 * Mixed templates still stringify replacements for embedding.
 */
function interpolate(template, context) {
  if (typeof template !== 'string') return template;

  const loneDollar = /^\$([a-zA-Z_][a-zA-Z0-9_.]*)$/.exec(template);
  if (loneDollar) {
    return resolveVar(loneDollar[1], context);
  }

  const loneBrace = /^\{\{\s*([^}]+)\s*\}\}$/.exec(template);
  if (loneBrace) {
    const value = evaluateExpr(loneBrace[1].trim(), context);
    return value == null ? '' : value;
  }

  let result = template.replace(/\{\{\s*([^}]+)\s*\}\}/g, (_, expr) => {
    return replacementToString(evaluateExpr(expr.trim(), context));
  });

  result = result.replace(/\$([a-zA-Z_][a-zA-Z0-9_.]*)/g, (_, varPath) => {
    return replacementToString(resolveVar(varPath, context));
  });
...
/**
 * Interpolate a `with` value. Strings are templates; numbers/bools stay as-is;
 * objects/arrays are walked so nested string fields still resolve.
 */
  ...
  try {
    switch (command) {
      case 'validate': {
        ...
        if (!type) {
          console.error(`✗ Cannot determine L7 type for ${path.basename(filePath)} (expected .flow, .tool, or .l7)`);
          process.exit(1);
        }
