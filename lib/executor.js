...
function loadFlow(name) {
  const entry = catalog.findFlow(name);
  if (!entry) {
...
function loadTool(name) {
  const entry = catalog.findTool(name);
  if (!entry) {
    return null;
  }
  return parseFile(entry.path);
...
/**
 * Audit actor. HTTP passes options.principal; CLI has none and stays executor/system.
 */
function auditWho(principal) {
  if (principal && principal.id) {
    return {
      entity_id: principal.id,
      role: principal.kind || 'system',
      loopback: principal.loopback === true
    };
  }
  return { entity_id: 'executor', role: 'system' };
}

/**
 * Execute a single tool call via MCP gateway
 */
async function executeTool(toolName, params, context, principal) {
  const tool = loadTool(toolName);
  const who = auditWho(principal);
 ...
 * Execute a do step
 */
async function executeDoStep(step, execState, context, rules, dryRun, principal) {
  ...
  if (step.each) {
    ...
    for (const item of items) {
      ...
      // Interpolate params — strings only; numbers/bools/records pass through
      const params = {};
      if (step.with) {
      ...
      if (dryRun) {
        ...
        try {
          const result = await executeTool(toolName, params, itemContext, principal);
  ...
  // Single execution — interpolate only string templates; do not String() records
  const params = {};
  if (step.with) {
    for (const [k, v] of Object.entries(step.with)) {
  ...
  return await executeTool(toolName, params, context, principal);
}
...
async function executeFlow(flowName, inputs = {}, options = {}) {
  const { dryRun = false, resume = null, principal = null } = options;
  ...
  for (let i = execState.step; i < flow.steps.length; i++) {
    ...
    try {
      if (step.do) {
        // Do step
        const result = await executeDoStep(step, execState, context, rules, dryRun, principal);
