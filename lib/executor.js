...
/**
 * Execute a single tool call (mock for now, real MCP integration later)
 */
async function executeTool(toolName, params, context) {
  ...
  // TODO: Wire to actual MCP servers via gateway
  console.log(`  → do: ${toolName}`);
  if (params) {
    for (const [k, v] of Object.entries(params)) {
      const resolved = interpolate(String(v), context);
  ...
  const mockResult = {
    ...
    count: 42,
    results: [
      { id: 1, name: 'Test 1', phone: '555-0001', consent: true },
      { id: 2, name: 'Test 2', phone: '555-0002', consent: true },
      { id: 3, name: 'Test 3', phone: '555-0003', consent: false }
    ],
  ...
  state.audit({
    who: { entity_id: 'executor', role: 'system' },
    what: { tool: toolName, result: 'success' }
  });
