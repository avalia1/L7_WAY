...
const loopbackHttp = require('./loopback-http');
...
async function boot() {
  ...
  const pulse = fieldTheory.startPulse({
    onBeat(report) {
      ...
      if (report.phases && report.phases.firing && report.phases.firing.nodes_fired > 0) {
...
async function execute(toolName, params = {}, options = {}) {
  ...
  // Domain is explicit (ADR 0001). 12D does not assign a folder.
  const domain = options.domain || null;

  let result;
  if (mode === 'mock') {
    result = executeViaMock(toolName, params);
  } else if (loopbackHttp.isHttpUrl(tool.server)) {
    result = await executeViaLoopback(tool, params);
  } else {
...
function loadTool(name) {
  ...
  const tool = yaml.load(fs.readFileSync(entry.path, 'utf8'));
...
async function executeViaLoopback(tool, params) {
  const url = loopbackHttp.assertLoopbackUrl(tool.server);
  const body = await loopbackHttp.postJson(url, {
...
function listTools() {
  return catalog.listTools()
    .map((entry) => {
      const tool = loadTool(entry.name);
      return {
        name: entry.name,
        description: tool?.description,
        server: tool?.server,
        does: tool?.does,
        // internal_projection: 12D field scoring only. Not identity. Not domain. ADR 0001.
...
function listPublicTools(opts) {
  return declaration.listPublicTools(opts);
}
