...
class DCFModel:
    ...
    def set_assumptions(
        ...
        revenue_growth: list = None,
        ebitda_margin: list = None,
        tax_rate: float = 0.25,
        capex_percent: list = None,
        nwc_percent: list = None,
        ...
        if ebitda_margin is None:
            ...
            if self.historical_financials:
                avg_margin = mean(self.historical_financials["ebitda_margin"])
    ...
    def project_cash_flows(self) -> dict[str, list]:
        """
    ...
    def sensitivity_analysis(
        self, variable1: str, range1: list, variable2: str, range2: list
    ) -> list:
        """
        ...
        """
        results = [[0.0 for _ in range2] for _ in range1]
    ...
    def generate_summary(self) -> str:
        ...
        summary = [
            ...
            f"  Revenue Growth: {mean(self.assumptions['revenue_growth']) * 100:.1f}% avg",
            f"  EBITDA Margin: {mean(self.assumptions['ebitda_margin']) * 100:.1f}% avg",
...
def calculate_beta(stock_returns: list, market_returns: list) -> float:
    ...
    covariance = sum((stock_returns[i] - mean_s) * (market_returns[i] - mean_m) for i in range(n)) / n
    market_variance = sum((x - mean_m) ** 2 for x in market_returns) / n
    beta = covariance / market_variance if market_variance != 0 else 1.0
    return beta


def calculate_fcf_cagr(fcf_series: list) -> float:
    """
